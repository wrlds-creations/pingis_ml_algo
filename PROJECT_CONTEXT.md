...
- `spel_retro_audio` T0012 built and installed the Review-only APK after Love approved USB install. The release APK was built from clean commit `52cfbb8` in short worktree `C:\st12`, verified to contain bundle strings `Spel-retro audio`, `spel_retro_audio_review_only`, and `multi_window_context_racket_weighted`, and installed on Motorola `ZY22L6NDHV` at `2026-06-02 15:27:29`. APK SHA256 is `3422CC2A34A0DAF31BDF03F89FF9CDE4BC2B0CDA7C972C0579D46B5B6C0D5A50`. It still only reclassifies saved candidates and cannot recover the 15 known missed Tomas/Stiga markers without T0013 candidate generation/surfacing.
...
- `spel_retro_audio`: Retro-analysis candidate family for post-recording playing-mode audio, separate from `studs_live`. Current selected candidate is `playing_retro_audio_rf_v2026_06_02_multi_window_context`, exported separately as `apps/collector/src/models/playing_retro_audio_model.json` and consumed by the opt-in `apps/collector/src/playingRetroAudio.ts` helper. T0011 wires it into a visible Review-only panel that must be run manually and does not alter marker truth automatically; T0012 installed that panel on Motorola. It is not a replacement for `audio_model.json`, and T0013 must add candidate generation/surfacing before it can recover events the saved candidate list missed.

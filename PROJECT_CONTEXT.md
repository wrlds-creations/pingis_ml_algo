...
- `spel_retro_audio`: Local retro-analysis candidate family for post-recording playing-mode audio, separate from `studs_live`. Current local candidate is `playing_retro_audio_rf_v2026_06_02_multi_window_context`; it is not bundled in `apps/collector/src/models/audio_model.json` and is not APK-ready.

...
- `spel_retro_audio`: Retro-analysis candidate family for post-recording playing-mode audio, separate from `studs_live`. Current selected candidate is `playing_retro_audio_rf_v2026_06_02_multi_window_context`, exported separately as `apps/collector/src/models/playing_retro_audio_model.json` and consumed only by the opt-in `apps/collector/src/playingRetroAudio.ts` helper. It is not a replacement for `audio_model.json`, not wired into visible Review UI yet, and not APK-installed.

...
- Date: `2026-06-02`
- Current branch: `codex/t0006-improve-playing-retro-audio-candidate`
- Current phase: `Audio-first playing-retro stabilization before app integration`
- Current status: `T0006 compared focused one-window variants; next active ticket is true multi-window/context features`
- Current ticket: `T0007`
- Last completed ticket: `T0006`
- Recommended next ticket: `T0007-playing-retro-audio-multi-window-context`
...
|---|---|---|---|
| `T0007` | Add true multi-window and non-leaky candidate-context features for `spel_retro_audio` | `Ready` | T0006 one-window weighting improved racket too little; next step must change the feature/candidate representation |

## Confirmed Next Tickets

| Ticket | Goal | Notes |
|---|---|---|
| `T0008` | Integrate `spel_retro_audio` behind a separate Review retro path | Only after candidate recall/false-positive gates are accepted |
| `T0009` | Revisit video-assisted FH/BH fusion after audio retro is stable | Keep video paused until audio is useful |
...
| `skills/pingis-audio-classification` | Audio ML/replay workflow | Primary area for T0004-T0007 |
...
- Build: `Not run for T0006; no app build needed`
- Tests: `npm run validate`, T0004 full report command, T0005 train/eval command, T0006 variant command, and targeted Python syntax checks passed on 2026-06-02
- Lint: `git diff --check` passed for T0006 on 2026-06-02
- Manual verification: `T0006 report compared three focused variants on audio_session_2026-05-29_002. Selected safe_racket_weighted improved holdout accuracy 0.759 -> 0.771, racket recall 0.604 -> 0.623, table recall 0.924 -> 0.933, and kept non-target recall 0.625. Aggressive weighting reached racket 0.679 but was rejected because non-target fell to 0.500.`
...
- `audio_session_2026-05-29_002` improves slightly with the T0006 safe one-window candidate, but racket recall remains too low for app integration.
...
- T0006 proves one-window reweighting is not enough; the next useful step is non-leaky multi-window/candidate-context features or better candidate timing.

"""
train_rf_audio_contact.py

Train the primary binary RandomForest contact model on a contact dataset CSV.

Run:
  python skills/pingis-audio-classification/scripts/train_rf_audio_contact.py
  python skills/pingis-audio-classification/scripts/train_rf_audio_contact.py --dataset <csv> --model-dir <dir>
"""
...
DEFAULT_DATASET = ROOT_DIR / "data" / "audio" / "processed" / "audio_contact_dataset.csv"
DEFAULT_MODEL_DIR = ROOT_DIR / "data" / "audio" / "models"

TARGET_LABELS = ["racket_contact", "not_racket_contact"]
META_COLS = {
    "label",
    "recorder_name",
    "session_id",
...
def make_group_split(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if "group_id" not in work.columns:
        work["group_id"] = work["source_file"].astype(str) if "source_file" in work.columns else work.index.astype(str)
    ...
    if min_groups_per_label < 2:
        raise ValueError("Need at least 2 unique recording groups per class for grouped train/test split.")
...
def build_group_cv_splits(train_df: pd.DataFrame) -> tuple[list[tuple[np.ndarray, np.ndarray]], int]:
    ...
    min_groups_per_label = int(groups["label"].value_counts().min())
    if min_groups_per_label < 2:
        raise ValueError("Need at least 2 unique recording groups per class in the training split for grouped CV.")
...
def get_scenario_breakdown(
    ...
    if "scenario_id" not in test_df.columns:
        return {"_error": {"message": "scenario_id missing in dataset"}}
...
def print_scenario_breakdown(breakdown: dict[str, dict]) -> None:
    if "_error" in breakdown:
        print("\nScenario breakdown:", breakdown["_error"]["message"])
        return

    print("\nScenario breakdown:")
    for scenario_id in SCENARIO_BREAKDOWN_IDS:
        item = breakdown.get(scenario_id, {"rows": 0})
        rows = int(item.get("rows", 0))
        if rows == 0:
            print(f"  {scenario_id}: no grouped test rows")
            continue
        print(
            f"  {scenario_id}: rows={rows} | exact_match={item['exact_match']:.3f}"
...
def main() -> None:
    ...
    parser.add_argument("--model-dir", default=str(DEFAULT_MODEL_DIR), help="Directory for saved model artifacts.")
    ...
    if df.empty:
        print("Contact dataset is empty. Review takes first or rerun preprocess with bootstrap.")
        return
    ...
    joblib.dump(final_clf, model_dir / "audio_contact_rf_classifier.pkl")
    joblib.dump(final_scaler, model_dir / "audio_contact_feature_scaler.pkl")
    joblib.dump(le, model_dir / "audio_contact_label_encoder.pkl")
    joblib.dump(feature_cols, model_dir / "audio_contact_feature_cols.pkl")
    print(f"\nFinal full-data contact model saved to {model_dir}")

    cm = confusion_matrix(y_test, y_pred)
    disp = ConfusionMatrixDisplay(cm, display_labels=le.classes_)
    disp.plot(colorbar=False)
    plt.title("Audio contact confusion matrix (grouped test)")
    plt.tight_layout()
    out_fig = model_dir / "audio_contact_confusion_matrix.png"
            ...
            json.dumps(
                {
                    "dataset": str(dataset),
                    "rows": int(len(df)),
                    "labels": {str(k): int(v) for k, v in counts.to_dict().items()},
                    "review_completed_rows": int(
                        df.get("review_completed", pd.Series([], dtype=bool)).fillna(False).sum()
                    ),

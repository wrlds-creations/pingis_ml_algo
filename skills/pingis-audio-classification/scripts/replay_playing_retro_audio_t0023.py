"""
Replay the T0022 joblib playing-retro model against reviewed dense sessions.

This is a local T0023 replay/tuning step. It does not retrain, export app JSON,
build an APK, or change studs_live.

Run:
  python skills/pingis-audio-classification/scripts/replay_playing_retro_audio_t0023.py
"""
...
from train_playing_retro_audio_t0022 import DATASET_CSV, MODEL_DIR, MODEL_ID
...
PREDICTIONS_CSV = EVAL_DIR / "playing_retro_audio_t0023_replay_predictions.csv"
EVAL_CSV = EVAL_DIR / "playing_retro_audio_t0023_replay_eval.csv"
SWEEP_CSV = EVAL_DIR / "playing_retro_audio_t0023_threshold_sweep.csv"
REPORT_JSON = EVAL_DIR / "playing_retro_audio_t0023_replay_report.json"
REPORT_MD = EVAL_DIR / "playing_retro_audio_t0023_replay_report.md"
...
def load_t0022_model(model_dir: Path) -> tuple[Any, Any, Any, list[str]]:
...
def predict_joblib_model(
    ...
    if missing:
        raise ValueError(f"T0022 replay dataset is missing features: {missing[:10]}")
    ...
    for racket_threshold in THRESHOLDS:
        for table_threshold in THRESHOLDS:
            predictions = prediction_events(
                candidate_df,
                prediction_column="t0022_prediction",
                confidence_column="t0022_confidence",
...
def recommendation(selected: dict[str, Any], baseline: dict[str, Any]) -> str:
    ...
    if tp_gain > 0 and fp_gain <= 0 and wrong_gain <= 0:
        return "proceed_t0024_export_with_selected_thresholds"
    if tp_gain >= 0 and fp_gain <= 2 and wrong_gain <= 1:
        return "proceed_t0024_export_cautiously_with_selected_thresholds"
...
def write_markdown(report: dict[str, Any], eval_df: pd.DataFrame) -> None:
    ...
    selected = report["marker_replay"]["t0022_selected"]
    candidate_rows = eval_df[(eval_df["kind"] == "candidate") & (eval_df["group"] == "all")]
    lines = [
        "# Playing Retro Audio T0023 Replay Report",
        "",
    ...
    lines.extend([
        ...
        "",
        "| Session | Baseline TP/Wrong/FP/Missed | T0022 TP/Wrong/FP/Missed |",
        "|---|---|---|",
    ])
    ...
    for session_id in report["sessions"]:
        ...
        t0022 = per_session["t0022_selected"].get(session_id, {})
        lines.append(
            f"| `{session_id}` | {base.get('true_positive', 0)}/{base.get('wrong_class', 0)}/"
            f"{base.get('false_positive', 0)}/{base.get('missed', 0)} | "
            f"{t0022.get('true_positive', 0)}/{t0022.get('wrong_class', 0)}/"
            f"{t0022.get('false_positive', 0)}/{t0022.get('missed', 0)} |"
        )
    ...
    lines.extend([
        ...
        "## 2026-06-03 Focus",
        "",
        f"- Baseline target candidate errors fixed by T0022 candidate replay: `{report['focus_2026_06_03']['t0022_correct_on_baseline_target_errors']}` / `{report['focus_2026_06_03']['baseline_target_candidate_errors']}`",
        f"- Baseline target rows called non-target fixed by T0022: `{report['focus_2026_06_03']['t0022_correct_on_baseline_non_target_target_rows']}` / `{report['focus_2026_06_03']['baseline_non_target_target_rows']}`",
        f"- Recovery/analysis rows selected by T0022 threshold: `{report['focus_2026_06_03']['selected_recovery_or_analysis_predictions']}`",
        "",
        "## Outputs",
        "",
...
def focus_summary(candidate_df: pd.DataFrame, selected_predictions: pd.DataFrame) -> dict[str, Any]:
    focus = candidate_df[candidate_df["session_id"].astype(str) == "audio_session_2026-06-03_005"].copy()
    ...
    t0022_correct = focus["t0022_prediction"].astype(str) == focus["label"].astype(str)
    selected_focus = selected_predictions[selected_predictions["session_id"].astype(str) == "audio_session_2026-06-03_005"]
    ...
    return {
        ...
        "t0022_correct_on_baseline_target_errors": int(
            t0022_correct.loc[target.index[baseline_errors.to_numpy()]].sum()
        ) if int(baseline_errors.sum()) else 0,
        "baseline_non_target_target_rows": int(baseline_non_target.sum()),
        "t0022_correct_on_baseline_non_target_target_rows": int(
            t0022_correct.loc[target.index[baseline_non_target.to_numpy()]].sum()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay T0022 playing-retro model and tune thresholds.")
...
def main() -> None:
    ...
    if not dataset_path.exists():
        raise SystemExit(f"Missing {dataset_path}; run T0022 training first.")
    model_dir = Path(args.model_dir)
    classifier, scaler, encoder, features = load_t0022_model(model_dir)
    ...
    candidate_df["t0022_prediction"] = predictions
    candidate_df["t0022_confidence"] = confidences
    ...
    for label, values in probabilities.items():
        candidate_df[f"t0022_probability_{label}"] = values
    ...
    for row in grouped_metric_rows("t0022_candidate_prediction", candidate_df, "t0022_prediction"):
        eval_rows.append({"kind": "candidate", **row})
    ...
    prediction_columns = [
        ...
        "t0022_prediction",
        "t0022_confidence",
        "t0022_probability_racket_contact",
        "t0022_probability_table_bounce",
        "t0022_probability_non_target",
        "source_rule",
        "match_outcome",
        "close_event_bucket",
        "neighbor_sequence",
    ...
    report = {
        ...
        "marker_replay": {
            ...
            "t0022_selected": selected_summary,
            "best_by_score": best_by_score,
            "per_session": {
                "baseline_visible": per_session_totals(baseline_session_rows),
                "t0022_selected": per_session_totals(selected_session_rows),
            },
        },
        "focus_2026_06_03": focus_summary(candidate_df, selected_events),
        "outputs": {
    ...
    print(
        "t0022_selected: racket_thr={racket} table_thr={table} pred={pred} tp={tp} wrong={wrong} fp={fp} missed={missed}".format(

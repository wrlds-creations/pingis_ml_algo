"""
Retrain the local T0022 playing-retro audio candidate.

This script is intentionally separate from the T0007 script so the 2026-06-03
...
Run:
  python skills/pingis-audio-classification/scripts/train_playing_retro_audio_t0022.py
"""
...
FOCUS_SESSION = "audio_session_2026-06-03_005"
MODEL_ID = "playing_retro_audio_rf_v2026_06_03_t0022_multi_window_context"

CANDIDATE_ROWS_CSV = OUT_DIR / "playing_retro_audio_candidate_rows_t0022_2026_06_03.csv"
DATASET_CSV = OUT_DIR / "playing_retro_audio_multi_window_dataset_t0022_2026_06_03.csv"
EVAL_CSV = EVAL_DIR / "playing_retro_audio_t0022_retrain_eval.csv"
PREDICTIONS_CSV = EVAL_DIR / "playing_retro_audio_t0022_retrain_predictions.csv"
REPORT_MD = EVAL_DIR / "playing_retro_audio_t0022_retrain_report.md"
...
REFERENCE_T0008 = {
    "audio_session_2026-05-28_002": {
        ...
        "table_bounce_recall": 0.932,
        "non_target_recall": 0.894,
    },
    "audio_session_2026-05-29_001": {
        "accuracy": 0.921,
    ...
    "audio_session_2026-05-29_002": {
        ...
        "table_bounce_recall": 0.933,
        "non_target_recall": 0.833,
    },
}
...
def evaluate_variant(
    ...
    for holdout_session in holdout_sessions:
        ...
        reference = REFERENCE_T0008.get(holdout_session)
        if reference:
            for metric in ["accuracy", "racket_contact_recall", "table_bounce_recall", "non_target_recall"]:
                value = as_float(summary.get(metric))
                summary[f"{metric}_vs_t0008"] = None if value is None else value - reference[metric]
    ...
    old_sessions = [item for item in session_summaries if item["session_id"] in REFERENCE_T0008]
    target_recalls = [
    ...
    old_reference_safe = all(
        (
            as_float(item.get("racket_contact_recall_vs_t0008")) is not None
            and as_float(item.get("table_bounce_recall_vs_t0008")) is not None
            and as_float(item.get("non_target_recall_vs_t0008")) is not None
            and float(item["racket_contact_recall_vs_t0008"]) >= -0.05
            and float(item["table_bounce_recall_vs_t0008"]) >= -0.03
            and float(item["non_target_recall_vs_t0008"]) >= -0.05
        )
...
def write_markdown(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Playing Retro Audio T0022 Retrain Report",
        "",
        ...
        "- Next step: T0023 should replay/tune this retrained model and recovery settings before any app export.",
        "",
        "## Data",
        "",
    ...
    lines.extend([
        ...
        "",
        "| Variant | Safe vs old T0008 | Score | Acc | Racket | Table | Non-target |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ])
    ...
    lines.extend([
        ...
        "",
        "| Holdout | Acc | Racket | Table | Non-target | Racket vs T0008 | Table vs T0008 | Non-target vs T0008 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ])
    for item in selected_summary["holdout_by_session"]:
        lines.append(
            ...
            f"{metric_text(item.get('racket_contact_recall_vs_t0008'))} | "
            f"{metric_text(item.get('table_bounce_recall_vs_t0008'))} | "
            f"{metric_text(item.get('non_target_recall_vs_t0008'))} |"
        )

    lines.extend([
        "",
        "## 2026-06-03 Focus Check",
        "",
        "This section is in-sample for the final saved model because T0022 intentionally trains the final candidate on the reviewed 2026-06-03 clip. Use T0023 replay for promotion evidence.",
        "",
        f"- Final 06-03 rows: `{focus['rows']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train local T0022 playing-retro audio candidate.")
...
def main() -> None:
    ...
    if not holdout_sessions:
        raise SystemExit("No requested holdout sessions exist in the T0022 dataset.")
    ...
    report: dict[str, Any] = {
        ...
        "training_choice": "fresh_multi_window_context_dataset_from_historical_playing_plus_2026_06_03",
